# frozen_string_literal: true

module StatusTag
  class Component < ApplicationComponent
    def initialize(statuses, id: nil, class_context: nil)
      super()
      @statuses = statuses
      @id = id
      @class_context = class_context
    end

    attr_reader :statuses, :id, :class_context

    def tags
      Array(statuses).map do |status|
        {
          text: I18n.t(status, scope: %i[components status_tag]),
          colour: COLOURS[status.to_sym],
          classes: class_context ? ["#{class_context}__tag"] : [],
          html_attributes: id ? { id: } : {},
        }
      end
    end

    COLOURS = {
      accepted: "green",
      active: "green",
      archived: "grey",
      assessment: "blue",
      assessment_in_progress: "blue",
      assessment_not_started: "grey",
      awarded: "green",
      cannot_start: "grey",
      completed: "green",
      confirmed_pending_checks: "teal",
      declined: "red",
      draft: "grey",
      error: "red",
      expired: "magenta",
      in_progress: "blue",
      invalid: "red",
      not_started: "grey",
      on_hold: "red",
      overdue: "magenta",
      overdue_consent: "magenta",
      overdue_ecctis: "magenta",
      overdue_further_information: "magenta",
      overdue_lops: "magenta",
      overdue_reference: "magenta",
      overdue_prioritisation_reference: "magenta",
      potential_duplicate_in_dqt: "magenta",
      pre_assessment: "magenta",
      preliminary_check: "magenta",
      prioritised: "purple",
      prioritisation_check: "magenta",
      received: "purple",
      received_and_overdue: "magenta",
      received_consent: "purple",
      received_decision_review: "purple",
      received_ecctis: "purple",
      received_further_information: "purple",
      received_lops: "purple",
      received_reference: "purple",
      received_prioritisation_reference: "purple",
      rejected: "red",
      requested: "yellow",
      review: "magenta",
      submitted: "grey",
      update_needed: "red",
      valid: "green",
      verification: "yellow",
      verification_in_progress: "blue",
      verification_not_started: "grey",
      waiting_on: "yellow",
      waiting_on_consent: "yellow",
      waiting_on_ecctis: "yellow",
      waiting_on_further_information: "yellow",
      waiting_on_lops: "yellow",
      waiting_on_reference: "yellow",
      waiting_on_prioritisation_reference: "yellow",
      withdrawn: "red",
    }.freeze
  end
end
