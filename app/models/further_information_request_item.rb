...
class FurtherInformationRequestItem < ApplicationRecord
  ...
  enum :information_type, { text: "text", document: "document", work_history_contact: "work_history_contact" }
  ...
  def completed?
    (text? && response.present?) || (document? && document.completed?) ||
      (work_history_contact? && contact_name.present? && contact_job.present? && contact_email.present?)
  end
