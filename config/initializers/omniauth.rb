...
onelogin_sign_in_issuer_uri = if Rails.configuration.gov_one.base_uri.present?
  URI(Rails.configuration.gov_one.base_uri)
end

callback_path = "/teacher/auth/gov_one/callback"

jwks_uri = onelogin_sign_in_issuer_uri.present? ? "#{onelogin_sign_in_issuer_uri&.to_s}.well-known/jwks.json" : nil

gov_one_redirect_uri = if HostingEnvironment.development?
  "http://localhost:3000#{callback_path}"
else
  "https://#{HostingEnvironment.host}#{callback_path}"
end

end_session_endpoint = onelogin_sign_in_issuer_uri.present? ? "#{onelogin_sign_in_issuer_uri&.to_s}logout" : nil

client_secret = if Rails.configuration.gov_one.base_62_private_key.present?
  OpenSSL::PKey::RSA.new(Base64.decode64(Rails.configuration.gov_one.base_62_private_key))
end
...
Rails.application.config.middleware.use OmniAuth::Builder do
  ...
  on_failure { |env| Teachers::OmniauthCallbacksController.action(:failure).call(env) }
end
