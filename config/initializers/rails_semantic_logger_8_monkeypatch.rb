...
unless RailsSemanticLogger::VERSION == LAST_TESTED_VERSION
  raise "rails_semantic_logger is version #{RailsSemanticLogger::VERSION} but " \
          "the monkey patch was last tested on #{LAST_TESTED_VERSION} - manually check " \
          "if it supports Rails 8 now and this can be removed, or that this still works as intended"
end
...
RailsSemanticLogger::ActiveRecord::LogSubscriber.include(
  LogSubscriberMonkeyPatch,
)
